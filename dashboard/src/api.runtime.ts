...
export type RuntimeEvalOverview = {
  ...
  acceptance: { status: string; release_ready: boolean; evidence_scope: string; generated_at: string | null; source_commit: string | null; blockers: string[]; case_count: number; holdout_count: number; holdout_passed: number; model_calls: number; approved_model_call_ceiling: number; cost_usd: number; duration_seconds: number; owner_accepted: boolean; owner_accepted_at: string | null; model_quality: { attempts: number; raw_passes: number; raw_failures: number; raw_pass_rate: number | null; model_responses?: number; response_rate?: number | null; provider_failures?: number; recoveries: number; recovery_rate: number | null } } | null
  next_action: string
}
