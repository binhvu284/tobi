...
import { SiAnthropic, SiGooglegemini, SiOpenrouter, SiOllama, SiX, type IconType } from '@icons-pack/react-simple-icons'
import type { AvailableModel } from '../../api'

/**
 * Premium model picker (Premium Chat v2 · checkpoint B).
 * A grouped dropdown: provider logos, models grouped by provider, and per-model
 * badges — context size, 👁 vision, reasoning, and a price/speed tier dot.
 */

const PROVIDER: Record<string, { name: string; color: string; Icon?: IconType }> = {
  anthropic:  { name: 'Anthropic',  color: '#D97757', Icon: SiAnthropic },
  openai:     { name: 'OpenAI',     color: '#10A37F' },
  gemini:     { name: 'Google Gemini', color: '#4285F4', Icon: SiGooglegemini },
  openrouter: { name: 'OpenRouter', color: '#8B7CF6', Icon: SiOpenrouter },
  grok:       { name: 'xAI Grok',   color: '#E5E7EB', Icon: SiX },
  ollama:     { name: 'Ollama',     color: '#E5E7EB', Icon: SiOllama },
  glm:        { name: 'GLM · Z.ai', color: '#3B82F6' },
  custom:     { name: 'Custom',     color: '#8B949E' },
}
const meta = (p: string) => PROVIDER[p] ?? { name: p, color: '#8B949E' }

export function ProviderLogo({ provider, size = 15 }: { provider: string; size?: number }) {
  const m = meta(provider); const Icon = m.Icon
  return (
    <span className="flex shrink-0 items-center justify-center rounded-md" style={{ width: size + 9, height: size + 9, background: `${m.color}22` }}>
      {Icon ? <Icon size={size} color={m.color} /> : <span className="text-[10px] font-bold" style={{ color: m.color }}>{m.name.slice(0, 1)}</span>}
    </span>
  )
}
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)}
        ...
        {current ? <ProviderLogo provider={current.provider} size={14} /> : <span className="flex h-[23px] w-[23px] items-center justify-center rounded-md bg-accent/15 text-accent"><Cpu size={13} /></span>}
              ...
              <div key={g.provider} className="mt-1">
                ...
                {g.items.map(m => (
                  <button key={m.id} onClick={() => pick(m.id)}
                    ...
                    {value === m.id ? <Check size={13} className="shrink-0 text-accent" /> : <span className="w-[13px] shrink-0" />}
