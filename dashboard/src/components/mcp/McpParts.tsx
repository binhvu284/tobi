// Extracted sub-components + shared styles from pages/Mcp.tsx.
...
export const btn = 'inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-xs font-medium text-muted transition-colors hover:border-accent/40 hover:text-text disabled:opacity-50'
export const btnPrimary = 'inline-flex items-center justify-center gap-1.5 rounded-md bg-accent/15 px-3 py-1.5 text-xs font-semibold text-accent transition-colors hover:bg-accent/25 disabled:opacity-50'
export const input = 'w-full rounded-md border border-border bg-bg px-3 py-2 text-sm text-text outline-none focus:border-accent/50'
...
export const PERM_CLS: Record<string, string> = {
...
export function IssueClientModal({ tools, onClose, onDone }: { tools: string[]; onClose: () => void; onDone: () => void }) {
...
export function AddConnectionModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  const [name, setName] = useState('')
...
export function TryItModal({ tool, onClose }: { tool: McpExternalTool; onClose: () => void }) {
...
// ── connection card ──────────────────────────────────────────────────────────
export function ConnectionCard({ conn, tools, reload }: { conn: McpConnection; tools: McpExternalTool[]; reload: () => void }) {
...
// ── OAuth config ─────────────────────────────────────────────────────────────
export function OAuthCard({ info, reload }: { info: McpServerInfo; reload: () => void }) {
  const o = info.oauth
...
export function TunnelCard({ info, reload }: { info: McpServerInfo; reload: () => void }) {
  const t = info.tunnel
  const [busy, setBusy] = useState(false)
  const toast = useToast().toast
...
// ── A2A tab ──────────────────────────────────────────────────────────────────
export function A2aTab({ selfCard, peers, reload }: { selfCard: A2aCard | null; peers: A2aPeer[]; reload: () => void }) {
...
export function MessagePeerModal({ peer, onClose }: { peer: A2aPeer; onClose: () => void }) {
