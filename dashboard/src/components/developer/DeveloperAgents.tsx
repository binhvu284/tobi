...
function statusTone(status: string) {
  ...
  if (status === 'future') return 'text-muted border-border bg-overlay/5'
...
function AgentRow({ profile, models, providers, routing, busy, onSave, onProbe, onLogin, onModels }: {
  ...
  const locked = profile.qualification?.configuration_locked === true
  const operational = profile.enabled && !locked
  ...
  useEffect(() => {
    ...
    profile.runner, profile.runner_mode, profile.qualification,
  ])
  ...
  const displayStatus = locked ? 'Future' : !profile.enabled ? 'Off' : profile.health_status === 'ready' ? 'Ready' : label(profile.health_status)
  ...
  const changeTool = (tool: DeveloperTool) => {
    ...
    setDraft(current => ({
      ...
      auth_mode: tool === 'codex' ? 'native_login' : 'inherited',
    }))
  }
  const save = async () => {
    setLocalBusy('save')
    ...
    <motion.section layout className={`relative overflow-visible rounded-md border transition-colors ${open ? 'z-40 border-accent/35 bg-surface/75 shadow-[0_18px_50px_rgb(0_0_0/0.12)]' : operational ? 'z-0 border-border/70 bg-surface/35 hover:border-border hover:bg-surface/60' : 'z-0 border-border/50 bg-surface/20 hover:border-border/70'}`}>
      <div className="flex min-h-16 w-full items-stretch">
        <button type="button" onClick={() => setOpen(value => !value)} className="grid min-w-0 flex-1 grid-cols-[auto_minmax(0,1fr)] items-center gap-3 px-3 text-left sm:grid-cols-[auto_minmax(150px,1fr)_120px_minmax(190px,1fr)] sm:px-4">
          ...
          <span className="min-w-0"><span className={`block truncate text-sm font-medium ${operational ? 'text-text' : 'text-muted'}`}>{profile.name}</span><span className="mt-0.5 block truncate text-[10px] text-muted sm:hidden">{developerToolName(toolFor(profile))} - {modelName}</span></span>
          <span className={`hidden w-fit rounded border px-2 py-0.5 text-[10px] font-medium sm:inline-flex ${statusTone(locked ? 'future' : profile.enabled ? profile.health_status : 'off')}`}>{displayStatus}</span>
        ...
        <div className="flex shrink-0 items-center gap-2 pr-3 sm:pr-4">
          <button
            ...
            aria-label={locked ? `${profile.name} is reserved for future development` : `${profile.enabled ? 'Deactivate' : 'Activate'} ${profile.name} for new goals`}
            title={locked ? 'Reserved for future development' : `${profile.enabled ? 'Deactivate' : 'Activate'} for new goals`}
          ...
          >
            <span className={`relative h-5 w-9 rounded-full border transition-colors ${operational ? 'border-success/60 bg-success/20' : 'border-border bg-background'}`}>
              <span className={`absolute top-0.5 h-3.5 w-3.5 rounded-full transition-all ${operational ? 'left-[17px] bg-success' : 'left-0.5 bg-muted'}`} />
            </span>
            <span className={`hidden text-[10px] font-medium lg:block ${operational ? 'text-success' : 'text-muted'}`}>{localBusy === 'toggle' ? <Loader2 size={12} className="animate-spin" /> : locked ? 'Future' : profile.enabled ? 'Active' : 'Off'}</span>
          </button>
      ...
      <AnimatePresence initial={false}>
        {open && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} className="overflow-visible">
          <div className="border-t border-border/60 px-4 py-5 sm:px-5">
            {locked && <div className="mb-4 rounded-md border border-border/70 bg-overlay/5 px-3 py-2.5 text-[11px] leading-5 text-muted"><span className="font-medium text-text">Reserved for future development.</span> {profile.qualification?.detail}</div>}
            <div className={`grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.8fr)] ${locked ? 'pointer-events-none opacity-45' : ''}`}>
              <div className="space-y-5">
                <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_150px]">
                  <label><span className="mb-1.5 block text-[11px] font-medium text-muted">Agent name</span><div className="relative"><Pencil size={13} className="absolute left-3 top-3 text-muted" /><input disabled={locked} value={draft.name} onChange={event => update('name', event.target.value)} className="h-10 w-full rounded-md border border-border bg-background pl-9 pr-3 text-sm text-text outline-none focus:border-accent disabled:cursor-not-allowed" /></div></label>
                  <label><span className="mb-1.5 block text-[11px] font-medium text-muted">Avatar</span><div className="relative"><UserRound size={13} className="absolute left-3 top-3 text-muted" /><input disabled={locked} value={String(draft.config?.avatar || '')} onChange={event => update('config', { ...draft.config, avatar: event.target.value.slice(0, 3) })} placeholder="TB" className="h-10 w-full rounded-md border border-border bg-background pl-9 pr-3 text-sm text-text outline-none focus:border-accent disabled:cursor-not-allowed" /></div></label>
                </div>
                <div><div className="mb-2 text-[11px] font-medium text-muted">Developer tool</div><div className="grid gap-2 sm:grid-cols-2">{toolOptions.map(option => {
                  ...
                  return <button key={option.id} type="button" disabled={locked || option.future || reviewer} onClick={() => changeTool(option.id)} className={`flex items-center gap-3 rounded-md border px-3 py-2.5 text-left transition-colors ${selected ? 'border-accent/45 bg-accent/10' : 'border-border/70 hover:border-border hover:bg-overlay/5'} disabled:cursor-default disabled:opacity-70`}><DeveloperToolLogo tool={option.id} size={16} /><span className="min-w-0 flex-1"><span className="flex items-center gap-2 text-xs font-medium text-text">{developerToolName(option.id)}{option.future && <span className="rounded bg-overlay/10 px-1.5 py-0.5 text-[9px] text-muted">Future</span>}</span><span className="mt-0.5 block text-[10px] text-muted">{option.detail}</span></span>{selected && <Check size={14} className="text-accent" />}</button>
                })}</div></div>
              </div>
            ...
            <div className="mt-5 flex flex-col gap-3 border-t border-border/60 pt-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0 text-[10px] leading-4 text-muted"><span className={`mr-2 inline-block h-2 w-2 rounded-full ${locked ? 'bg-muted' : profile.health_status === 'ready' ? 'bg-success' : 'bg-warning'}`} />{locked ? profile.qualification?.detail : profile.health_detail || 'Run a connection test before assigning this agent.'}</div>
              <div className="flex shrink-0 gap-2"><button type="button" onClick={() => void test()} disabled={working || locked} className="inline-flex h-9 items-center gap-2 rounded-md border border-border px-3 text-xs text-text hover:bg-overlay/5 disabled:opacity-40">{localBusy === 'test' ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />} Test agent</button>{dirty && <button type="button" onClick={() => void save()} disabled={working || locked || draft.name.trim().length < 2} className="inline-flex h-9 items-center gap-2 rounded-md bg-accent px-3 text-xs font-medium text-background disabled:opacity-40">{localBusy === 'save' ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Save changes</button>}</div>
            </div>
          </div>
        </motion.div>}
      </AnimatePresence>
