...
export default function GraphToolbar(p: Props) {
  ...
  return (
    <div className="absolute left-0 right-0 top-0 z-10 flex flex-col gap-2 border-b border-border bg-surface/80 px-3 py-2 backdrop-blur">
          ...
          {DOMAIN_ORDER.map(d => {
            ...
            return (
              <button key={d} onClick={() => p.onDomain(d)}
                className={`rounded-lg px-2.5 py-1 text-xs transition-colors ${active ? 'bg-accent/15 text-accent ring-1 ring-accent/40' : 'text-muted hover:text-text'}`}>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <div className="relative">
            ...
            <input value={q} onChange={e => runSearch(e.target.value)} placeholder="Search · fly-to"
              className="w-44 rounded-lg border border-border bg-bg py-1.5 pl-7 pr-6 text-xs text-text outline-none focus:border-accent/50" />
            ...
            {results.length > 0 && (
              <div className="absolute right-0 top-full z-20 mt-1 max-h-64 w-64 overflow-y-auto rounded-lg border border-border bg-surface shadow-xl">
                {results.map(r => (
                  <button key={r.id} onClick={() => { p.onFocusResult(r.id); setResults([]); setQ('') }}
                    className="flex w-full items-center justify-between gap-2 px-3 py-1.5 text-left text-xs text-text hover:bg-accent/10">
          ...
          <button onClick={() => setShowFilters(s => !s)} title="Filters"
            className={`rounded-lg border p-1.5 ${showFilters ? 'border-accent/50 text-accent' : 'border-border text-muted hover:text-text'}`}>
            <SlidersHorizontal size={14} />
          </button>
          <button onClick={p.onToggleConnect} title="Connect two nodes"
            className={`rounded-lg border p-1.5 ${p.connectMode ? 'border-purple/60 bg-purple/15 text-purple' : 'border-border text-muted hover:text-text'}`}>
            <Link2 size={14} />
          </button>
          <button onClick={p.onAddNode} title="Add node"
            className="rounded-lg border border-border p-1.5 text-muted hover:text-text"><Plus size={14} /></button>
          <button onClick={p.onTogglePerformance} title={p.performance ? 'Performance mode ON' : 'Full neuron FX'}
            className={`rounded-lg border p-1.5 ${p.performance ? 'border-warning/50 text-warning' : 'border-border text-muted hover:text-text'}`}>
            {p.performance ? <ZapOff size={14} /> : <Zap size={14} />}
          </button>
          <button onClick={p.onRefresh} disabled={p.syncing} title="Sync + refresh"
            className="flex items-center gap-1.5 rounded-lg border border-accent/40 bg-accent/10 px-2.5 py-1.5 text-xs font-semibold text-accent hover:bg-accent/20 disabled:opacity-50">
      ...
      {showFilters && (
        <div className="flex flex-wrap items-center gap-3 border-t border-border/60 pt-2 text-xs text-muted">
          ...
          <label className="flex items-center gap-1.5">
            ...
            <input value={p.category} onChange={e => p.onCategory(e.target.value)} placeholder="any"
              className="w-28 rounded border border-border bg-bg px-2 py-1 text-text outline-none focus:border-accent/50" />
          </label>
          <label className="flex items-center gap-1.5">
