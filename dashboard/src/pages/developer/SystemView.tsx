...
export function LearningView({ state, busy, onReplay }: {
  ...
  return <div className="space-y-8">
    <section className="flex flex-col justify-between gap-3 border-y border-border py-5 sm:flex-row sm:items-center"><div><div className="flex items-center gap-2"><BookOpen size={16} className="text-accent" /><h2 className="text-sm font-semibold text-text">Evidence-backed improvement</h2></div><p className="mt-1 text-xs text-muted">{state.records.length} outcomes · {state.playbooks.length} reusable playbooks</p></div><ActionButton onAction={onReplay} busy={busy} disabled={!state.playbooks.length} icon={<TestTube2 size={14} />} className="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-accent/40 px-3 text-sm text-accent"> Replay evaluations</ActionButton></section>
  ...
  return (
    <section className="border-y border-border py-5">
      ...
      <div className="mt-6 flex flex-col gap-2 border-t border-border pt-4 sm:flex-row sm:items-end">
        ...
        <ActionButton busy={busy} disabled={eligible === 0 || master.length < 6} icon={<RotateCcw size={14} />} onAction={async () => { await onCleanup(master); setMaster('') }} className="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-border px-3 text-sm text-text hover:bg-overlay/5"> Clean eligible</ActionButton>
      </div>
    </section>
  )
}
