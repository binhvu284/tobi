...
|---|----|------|-------------|--------|-------|
| 31 | `FIX-CORE-8H-002` | [**Pages tell you when they fail to load**](PAGES_REPORT_LOAD_FAILURES_SPEC.md) | A page that cannot load its data says so, instead of just looking empty. | ⚪ Pending (spec written - awaiting approval) | 65 silent `.catch(() => {})` across 23 files. Adds one shared `LoadFailure` piece plus a guard test, and unblocks the 4s freeze in `/api/health`. [history](QUEUE_DELIVERY_LOG.md#item-31) |
| 30 | `CHECK-CHAT-2H-006` | [**Chat self-check**](CHAT_SELF_CHECK_SPEC.md) | One button that proves Chat really works, not just that a model replies. | ⚪ Pending (spec approved - awaiting go-ahead) | Approved as a spec. Nothing built yet. [history](QUEUE_DELIVERY_LOG.md#item-30) |
| 29 | `CHECK-DEV-8H-005` | [**Fallback recovery test**](FALLBACK_RECOVERY_TEST_PLAN.md) | Checks TOBI recovers properly when a step fails midway. | ⚪ Draft (not started) | Created by the Developer page itself. [history](QUEUE_DELIVERY_LOG.md#item-29) |
...
| 21 | `FOUND_BASE-CORE-5D20H-002` | [**Mission Control Infrastructure V2**](MISSION_CONTROL_INFRASTRUCTURE_V2_PLAN.md) | One engine behind every request, so nothing is lost when something crashes. | 🟠 In progress (v2 - 90%) | T08 Run 2B accepted; Run 3A checkpoint-recovery extraction planned. Do not edit core/runtime/* or core/schema/runtime.py. [history](QUEUE_DELIVERY_LOG.md#item-21) |
