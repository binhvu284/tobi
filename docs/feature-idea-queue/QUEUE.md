...
> #34/T08 repair is committed and pushed through `685a1a8`; release remains blocked pending live
> model-quality proof and owner acceptance.

| # | ID | Name | Description | Status | Notes |
|---|----|------|-------------|--------|-------|
| 34 | `UPG-CORE-2D12H-011` | [**TOBIval Operational Intelligence and Model Independence**](TOBIVAL_OPERATIONAL_INTELLIGENCE_PLAN.md) | Real exams prove TOBI's quality, while common MC work relies less on the selected model. | 🟠 In progress (live proof required) | T08 repairs the overclaimed T07 evidence path. All 72 cases, including 14 holdouts, now use canonical Runtime execution with five-stage decision provenance. The offline artifact reports ECR `100` and scoped LDR `8.8021`, but `0` model responses and `156` provider failures mean live model quality is unproven; release is blocked by `model-quality-proof-missing`. Deterministic recovery is `100%`, the shared gate passes `29/29`, and the dashboard build passes. Production routing is integrated only for narrow safe workflows with no required fields. A 156-call live rerun and owner dashboard acceptance remain. [history](QUEUE_DELIVERY_LOG.md#item-34) |
