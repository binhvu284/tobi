...
> Queue snapshot refreshed 2026-08-25. #21 is committed and complete; #33 is present in the
> current worktree but remains local until its active package is committed and gated. Owner-approved
> #34 planning is ready, but implementation must wait for #33 closure and the T00 baseline lock.

| # | ID | Name | Description | Status | Notes |
|---|----|------|-------------|--------|-------|
| 34 | `UPG-CORE-2D12H-011` | [**TOBIval Operational Intelligence and Model Independence**](TOBIVAL_OPERATIONAL_INTELLIGENCE_PLAN.md) | Real exams prove TOBI's quality, while common MC work relies less on the selected model. | 🟡 Queued (owner-approved plan) | T00 must freeze 72 total cases including 14 holdouts, metric formulas, exact strong/weak models, and the unchanged-code baseline before production work. Target: Eval Completion >=90% and LLM Dependency <=50% for supported MC workflows. Wait for #33 closure; absorbs #29. [history](QUEUE_DELIVERY_LOG.md#item-34) |
| 33 | `CHECK-CORE-4H-007` | [**One-click infrastructure test**](INFRASTRUCTURE_SELF_CHECK_SPEC.md) | One button on Health that proves the whole engine works on this machine. | 🟠 In progress (local worktree; not committed) | Health -> Infrastructure implementation and 23-suite evidence exist locally, but the package is still under #21/T15 gate review. Do not treat it as shipped until commit and gate evidence are recorded. [history](QUEUE_DELIVERY_LOG.md#item-33) |
