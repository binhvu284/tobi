...
> #34/T00 is owner-accepted; T01 through T06 are green. T07 final acceptance is next.

| # | ID | Name | Description | Status | Notes |
|---|----|------|-------------|--------|-------|
| 34 | `UPG-CORE-2D12H-011` | [**TOBIval Operational Intelligence and Model Independence**](TOBIVAL_OPERATIONAL_INTELLIGENCE_PLAN.md) | Real exams prove TOBI's quality, while common MC work relies less on the selected model. | 🟠 In progress (T06 green locally) | T00 is owner-accepted. T01-T05 provide real scoring, deterministic workflows, grounded outcomes, and enforced scoped gates. T06 adds a private Eval API and an owner-facing Eval Control Center with bounded evidence, truthful missing-proof states, and desktop/mobile coverage. Normal Chat runs no suite and all Runtime flags remain off. Focused Gate `3/3`; 40 backend checks, production build, and Playwright pass. T07 final acceptance is next. [history](QUEUE_DELIVERY_LOG.md#item-34) |
