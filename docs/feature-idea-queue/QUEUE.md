...
> Queue snapshot refreshed 2026-08-30. #21 and #33 are committed and green.
> #34/T08 live proof is complete against source `d1a3448`; owner dashboard acceptance remains.
> #35 is owner-approved and ready to start only after #34 reaches Done.

| # | ID | Name | Description | Status | Notes |
|---|----|------|-------------|--------|-------|
| 35 | `UPG-CORE-8D32H-012` | [**TOBI Agent Tier Completion**](AGENT_TIER_COMPLETION_PLAN.md) | TOBI finishes assigned digital work safely and proves exactly what happened. | 🟡 Ready (blocked by #34) | Owner-approved direction and worker plan. T02A adds confirmed Chat-to-Developer dispatch, live run tracking, and session outputs to Coding maintenance without creating another queue or Runtime. The item qualifies seven abilities and five workflow families through 30 frozen cases, five holdouts, and 20 real MC runs. Start only after #34 owner acceptance and Done status. Operator remains the next tier. [history](QUEUE_DELIVERY_LOG.md#item-35) |
| 34 | `UPG-CORE-2D12H-011` | [**TOBIval Operational Intelligence and Model Independence**](TOBIVAL_OPERATIONAL_INTELLIGENCE_PLAN.md) | Real exams prove TOBI's quality, while common MC work relies less on the selected model. | 🟠 In progress (owner acceptance required) | T08 repairs the overclaimed T07 evidence path. The approved live rerun used all 72 frozen cases and 14 holdouts without changing the baseline. It recorded 156/156 model responses, ECR `100`, scoped LDR `8.8021`, raw model pass `32.0513%`, deterministic recovery `67.9487%`, no provider failures, and no artifact blocker. The default owner CLI, shared 29-suite gate, dashboard build, and desktop/mobile Eval checks pass. Production routing remains limited to narrow safe workflows with no required fields. Owner dashboard acceptance is the only #34 closure gate left. [history](QUEUE_DELIVERY_LOG.md#item-34) |
...
| 23 | `UPG-NEWS-3D-009` | [**News Page V2**](NEWS_PAGE_V2_PLAN.md) | A personal news feed that learns what you like and tells you why it showed it. | 🟠 In progress (v2 - 90%) | Flag is off. Owner must do visual QA and a 20-item evidence review before flipping it. [history](QUEUE_DELIVERY_LOG.md#item-23) |
