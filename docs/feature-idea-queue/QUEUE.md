...
|---|----|------|-------------|--------|-------|
| 32 | `UPG-CORE-3H-010` | [**Health checks run together**](HEALTH_CHECKS_CONCURRENT_SPEC.md) | The health button finishes in one check's time, not all of them added up. | ⚪ Pending (spec written - awaiting approval) | Measured 20-50s, sequential, showing nothing until the end. A regression from #30's honest chat check (18.5s of it). Runs them concurrently and streams each row as it lands. Also takes two synchronous network calls off the event loop - structural, no symptom reproduced. [history](QUEUE_DELIVERY_LOG.md#item-32) |
