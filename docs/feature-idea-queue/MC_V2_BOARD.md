...
**In progress.** T00 through T06 and T07 Runs 1-2B are accepted. Run 3A is delivered, and T07 remains open.

**Delivered:** 7 complete packages plus T07 Runs 1-3A, about **79-86%** of total effort.

**Next:** review and accept T07 Run 3A, then plan Run 3B. The dormant adapter exposes terminal status
and fixed read-only identity/location/version commands only. Central policy plus two checks by the
existing terminal gate block unsafe, disabled, wrong-mode, or reclassified calls. Mutable and
background commands stay in Run 3B, so T07 and T08 remain closed to release.
...
| T07 | **Move the first real tools over:** files, terminal, projects. Each mutation gets a receipt, and a retry cannot double-apply it. | T03, T06 | High | Runs 1-2B accepted; Run 3A delivered; review |
