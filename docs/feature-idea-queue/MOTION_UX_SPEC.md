...
> **Queue status:** ✅ Done (v1) — **M1–M4 all shipped** (build clean) · **Scope:** v1 = global motion system + all 12 pages (phased M1–M4) · **Owner-reviewed:** 30 Q&A captured below
>
> **M1 done:** `src/lib/motion.ts` (tokens + level-aware variants), `src/context/MotionProvider.tsx` (Full/Reduced/Off, merges OS `prefers-reduced-motion`, writes `data-motion`), `src/components/motion/*` (`Reveal`, `Stagger`/`StaggerItem`, `PageBoot`, `Scanline`, `AmbientField`, `SpotlightCard`, `CountUp`, `TraceButton`), `index.css` motion tokens/keyframes + `[data-motion]` reduced-motion guards + `[data-theme-anim]` crossfade, `<PageBoot>` route transition wired in AppShell (keyed per path, no exit-gating), sliding `layoutId` nav pill + icon pop (namespaced via `LayoutGroup`), ThemeProvider 300ms crossfade window, Settings **Motion** toggle.
>
> **M2 done (the 3 signature effects):** **Brain neural ingestion** (`components/brain/NeuralIngestion.tsx` — converging particles → glowing orb, live typed pipeline log with real extracted count, animated progress bar; cards reveal via `Stagger`); **Chat orb + phases** (`ThinkingOrb` in `Chat.tsx` — pulsing orb + `Recalling…`→`Connecting…`→`Thinking…`, streamed reply leads with a blinking caret); **Health diagnostic sweep** (`Health.tsx` — radar-scan rows while testing, then staggered results each snapping green/red with a `SnapRing` glow; overall HP `CountUp`). All reduced/off-aware.
>
> **M3 done (marquee pages):** **Dashboard** — once-per-session `HeroBoot` "SYSTEM ONLINE" overlay + `AmbientField` atmosphere + KPI `SpotlightCard`/`CountUp` + Launchpad `TraceButton`; **Command Palette** — result `Stagger` cascade + sliding `layoutId` selection glow (on top of the existing spring-in); **Evolution** — refined glow-sweep across the unlocking tier node + confident success toast (with the existing rings/emblem/sound); **Office** — breathe/blink + desk glow already shipped in queue #3, so Decision #22 needs no rebuild.
>
> **M4 done (remaining pages + polish):** **ToastProvider** — colored edge bar + bottom drain timer (`scaleX`, matches 4.2s dismiss) + slide/spring (Decision #19); **atmosphere rollout** — `AmbientField` (re-anchored to `z-index:-1`) on Ability/Architecture/Task/Projects/ControlRoom; **Task** metrics → `SpotlightCard` + `CountUp`; every other route inherits the global `<PageBoot>` transition + nav slide + theme crossfade from M1 (Inbox/Integrations/Mcp left at the baseline by design). **60fps audit:** transform/opacity throughout; paint-based bits are brief gated one-shots; all decorative loops killed under reduced/off; no lingering `will-change`. **#6 complete — all four milestones shipped, ≈+20kB total.**
