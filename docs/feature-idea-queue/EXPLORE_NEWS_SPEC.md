# Explore → News — Feature Spec & Requirements

> **Status:** 🟡 Queued — Phase 1 (Requirements) COMPLETE. 28 locked decisions (E1–E28) from a
> single UI-picker question round. **Not built.** Portable handover in the same format as the other
> queue specs; paste into a fresh session to begin the build, gated behind Thomas's approval.
>
> **Owner:** Thomas (sole principal, inherited `[D66]`). **Head agent:** TOBI.
> **Date:** 2026-06-26. **Awaiting two inputs** before build — see §8.
>
> **Related specs:** reaches via [CONDUCTOR_SPEC.md](CONDUCTOR_SPEC.md) (#7, conversational
> "summarize today's news"); secrets via [GENESIS_SPEC.md](GENESIS_SPEC.md) (#4, encrypted vault);
> interest model can seed from [BRAIN_SPEC.md](BRAIN_SPEC.md) (#1, owner profile).
...
summarize → rank, the same hub-and-spoke spirit as the Office and the TOBI Conductor engine #7).
**News** is the first page.
...
- **Conductor = Tobi-mediated pipeline `[E2]`.** Not static API embeds — TOBI orchestrates
  **fetch → dedupe → summarize → rank** for every tab. Shares the conversational surface of the
  **TOBI Conductor engine (#7)** so the page is reachable by command, not a new visualized mode.
...
- **No autonomous push `[E8]`.** TOBI does **not** interrupt with Telegram alerts. It surfaces News
  only **on request** through the Conductor engine (#7) / Hermes gateway — e.g. *"Hey TOBI,
  summarize today's news for me"* → TOBI returns the digest in chat. (One bot only `[H16]`.)
- **Secrets `[E25]`.** All new API keys live in the **Genesis encrypted vault (#4,** `core/vault.py`
  **+ `/integrations` page)** with a Settings/Explore source panel (connect / toggle / status).
  Reference key **names** only, never values (inherited guardrail).
...
- **Owner-tunable algorithm `[E18]` (knobs in the Explore source panel):** **source weights** ·
  ...
  **editable interest prompt** (the natural-language "is this interesting to me" prompt TOBI uses —
  can seed from the Brain owner profile #1).
...
```
LLM calls reuse **`llm_usage`**; keys reuse the **Genesis vault** (#4).
...
- **Phase A — Backbone + Models (read, cached):** Explore section + AppShell nav, ingestion job +
  scheduler hook, `explore_*` tables, Genesis-vault source panel, news backbone + headlines rail,
  **Models tab** (leaderboard + compare + new-model freshness). Ships visible value, de-risks the
  pipeline.
- **Phase B — Tools + Social:** trending Tools tab (HN/PH/GitHub + AI-judge), Social-Trending feed
  (Reddit + Tavily) with the algorithm knobs + source toggles.
- **Phase C — Conductor polish:** editorial "TOBI's take" + on-request digest via #7 ("summarize
  today's news"), GDELT time-metric charts, X opt-in path, mute/keyword refinements.

> Each phase ends with: `npm --prefix dashboard run build` green + a Playwright screenshot pass +
> a backend smoke (`curl` the new endpoints against the real DB) — same gate as the other specs.
...
All keys are stored/served through the **Genesis vault (#4)** — never committed, never logged.
...
| E2 | Conductor | Tobi-mediated **fetch→dedupe→summarize→rank**; shares Conductor #7 surface |
...
| E8 | Push | **No autonomous push**; on-request only via Conductor #7 ("summarize today's news") |
...
| E25 | Secrets+lang | Keys in **Genesis vault (#4)** + source panel; **English, VN-weighted** |
...
*End of spec. Sufficient to begin the build once Thomas approves and provides the §8 inputs.*
