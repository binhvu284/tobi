...
| Tests | Focused tracked suites cover the established runtime, security, UI, and compatibility paths. #34/T00 preserves the accepted unchanged-code baseline: ECR `50` and LLM Dependency `85.5769`. The approved T08 rerun executed all 72 cases, including 14 holdouts, through canonical Runtime runs and made 156 bounded Codex subscription calls. All 156 returned; raw model output passed `32.0513%`, deterministic recovery handled `67.9487%`, final ECR is `100`, and scoped LDR is `8.8021`. The artifact has no blocker and reports `release_ready=true`; the owner accepted that exact artifact on 2026-08-30. #35/T01 adds 17 focused Agent-registry checks; the shared gate now contains 31 suites. The dashboard production build and desktop/mobile Playwright checks pass. |
...
- #18 TOBI Coding Agent v1 remains the base continuous goal/lease/worktree/review/deployment system. #22 Coding Agent V2 is qualified for the Codex-only path; its target-VPS soak remains a deployment gate. #21 Mission Control Infrastructure V2, #33 Infrastructure self-check, and #34 TOBIval are complete. #34 owner acceptance is bound to the exact live-proof artifact. For #35 Agent Tier Completion, the owner accepted T00's exact baseline for production revision `fc4d6d7`, dataset `17dda4f3...`, and artifact `9cddb15a...`. T01 replaces the legacy static Tier II checklist with one persisted seven-ability evidence registry. Evolution now reports evidence, freshness, missing proof, and the next owner action from that registry. Its correct initial result remains 0/7 because T02-T06 have not yet produced qualifying workflow evidence. The T01 checks pass 17/17 and the shared gate passes 31/31. The separately delivered DeepSeek Harness remains outside #35 and its files were not modified or staged. Legacy deletion is explicitly deferred to a separate owner-approved exit review.
