...
- Mission Control Runtime V2 has local foundations for immutable ordered run/System history, secret-redacted payloads, deterministic projections, canonical runs/plans, versioned state, exclusive leases, restart checkpoints, bounded retries, typed recovery, persisted loop limits, exact-once usage, action receipts, completed replay, crash reconciliation, dormant central policy decisions, and a dormant canonical tool registry. The registry enforces versioned MCP-compatible identities, strict JSON Schema 2020-12 input/output contracts, no remote schema references, explicit fail-closed availability, and deterministic bounded allowlist-only discovery. Pure adapters now convert existing Conductor/Chat specs, public inbound FastMCP definitions, and persisted outbound MCP input schemas into isolated canonical snapshots. External tool identities include stable connection ids, schema content controls contract version, external behavior stays conservatively high-risk, and the read-only outbound snapshot excludes endpoints and credential references. Policy contracts now produce deterministic `allow`, `require_approval`, or `deny` results from typed mode, surface, risk, side-effect, permission, integration, credential-readiness, trust, certainty, isolation, budget, approval, and kill-switch facts. Migration 007 stores each engine-verified decision plus an ordered `policy.decided` event in immutable redacted history. Migration 008 adds durable single-winner owner approvals. The metadata-only policy-facts adapter now distinguishes missing, locked, unavailable, unknown, and purpose-mismatched credentials without retrieving values, and maps normalized Chat/Terminal modes conservatively so plan or unknown Terminal modes deny. `runtime.v2_policy` and `runtime.v2_tools` remain off and no live caller uses these services yet. Chat and Agent call the canonical gateway when Runtime V2 events are enabled. Plain-text direct Chat becomes canonical only when the Chat runtime plus the default-off events, global execution, and Chat execution gates all pass: the gateway owns acknowledgement, one leased `respond` step, durable success/failure, and exact replay of the privately linked assistant message, while Conductor remains the compatibility response engine. Attachments, read/tool Chat, and Agent remain shadow/legacy. Turning off `runtime.v2_chat_execution` rolls new direct-Chat work back; already accepted runs retain their persisted mode. Agent recovery reuses its linked canonical run. `GET /api/runtime/runs/{run_id}/events` provides session-scoped, flag-gated, bounded, redacted SSE replay from `after` or `Last-Event-ID`, including live tailing and duplicate-free terminal reconnect.
