...
- #18 TOBI Coding Agent v1 remains the base continuous goal/lease/worktree/review/deployment system. #22 Coding Agent V2 is qualified for the Codex-only path; its target-VPS soak remains a deployment gate. #21 Mission Control Infrastructure V2, #33 Infrastructure self-check, and #34 TOBIval are complete. #34 owner acceptance is bound to the exact live-proof artifact. #35 Agent Tier Completion has a prepared T00 Eval-only worker contract and is ready for red-first implementation; production behavior is unchanged, and baseline capture waits for the current uncommitted Developer/worker production changes to settle. Legacy deletion is explicitly deferred to a separate owner-approved exit review.
