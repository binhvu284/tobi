...
| TOBIval operational layer | `core/runtime/eval_dataset.py`, `eval_scorers.py`, `eval_runner.py`, `eval_metrics.py`, `workflows.py`, `typed_resolution.py`, `grounded_outcomes.py` | Hash-locked cases, executable scoring, immutable result evidence, deterministic workflows, current entity lookup, typed arguments, grounded no-model outcomes, and bounded model recovery | T02-T04 expose additive adapters and stable evidence references; they do not replace production `route_turn` |
