...
| MC Runtime V2 foundation | `core/runtime/contracts.py`, `event_store.py`, `projections.py`, `rebuild.py`, `repository.py`, `state.py` | Shared validated contracts, append-only ordered events, redaction, deterministic rebuilds, and canonical run/plan persistence | Dormant through T03 Run 1; requests, plans, and effective loop policies persist atomically with legal version-checked state changes, but no live surface uses the repository yet |
