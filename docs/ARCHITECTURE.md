...
| MC Runtime V2 foundation | `core/runtime/contracts.py`, `event_store.py`, `projections.py`, `rebuild.py`, `repository.py`, `control.py`, `state.py` | Shared validated contracts, append-only ordered events/checkpoints, redaction, deterministic rebuilds, canonical run/plan persistence, exclusive step leases, bounded retries, and persisted recovery control | Dormant through T03 Run 3A; cancellation fences active workers and recovery commands are consumed once, but no live surface uses the repository yet |
...
| MC Runtime V2 | `mc_run_events`, `mc_change_events`, `mc_runtime_projections`, `mc_system_entities`, `mc_system_edges`, `mc_runs`, `mc_run_steps`, `mc_run_checkpoints`, `mc_run_commands`, `mc_loop_recipes`, `mc_loop_runs` |
