...
| TOBIval operational layer | `core/runtime/eval_dataset.py`, `eval_scorers.py`, `eval_runner.py`, `eval_metrics.py`, `workflows.py` | Hash-locked cases, executable scoring, immutable result evidence, deterministic supported-workflow selection, and model/tool boundaries | T02 exposes an additive classifier/Chat adapter but does not replace production `route_turn`; T03 must resolve typed fields first |
