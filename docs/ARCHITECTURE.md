...
| MC Runtime V2 foundation | `core/runtime/contracts.py`, `event_store.py`, `projections.py`, `rebuild.py`, `repository.py`, `state.py` | Shared validated contracts, append-only ordered events/checkpoints, redaction, deterministic rebuilds, canonical run/plan persistence, and exclusive step leases | Dormant through T03 Run 2; expired workers are fenced from writes and a new worker receives the latest durable checkpoint, but no live surface uses the repository yet |
