...
| TOBIval operational layer | `core/runtime/eval_dataset.py`, `eval_scorers.py`, `eval_runner.py`, `eval_metrics.py`, `workflows.py`, `typed_resolution.py` | Hash-locked cases, executable scoring, immutable result evidence, deterministic workflows, current entity lookup, typed arguments, and model/tool boundaries | T02-T03 expose additive adapters and stable accepted-request hashes; they do not replace production `route_turn` |
