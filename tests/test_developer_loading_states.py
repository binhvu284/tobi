"""Guard the loading affordance on every asynchronous Developer control.
...
These checks fail while a Developer control can trigger async work with no pending state.
...
SURFACE = [
    ...
    SRC / "pages" / "developer" / "SystemView.tsx",
    SRC / "pages" / "developer" / "WorkflowHeader.tsx",
    SRC / "components" / "developer" / "DeveloperProcess.tsx",
    SRC / "components" / "developer" / "DeveloperQueue.tsx",
    SRC / "components" / "developer" / "DeveloperAgents.tsx",
    SRC / "components" / "developer" / "DevelopmentGoals.tsx",
]
...
for path in SURFACE:
    ...
    hits = [tag for tag in tags if ASYNC_HANDLER.search(tag) and "disabled" not in tag]
    ok(f"{path.name}: every async button carries a pending state", not hits,
       f"{len(hits)} raw <button> with an async onClick and no disabled= (use ActionButton): "
       + " | ".join(tag[:90] for tag in hits[:2]))
...
print(f"\n{'ALL' if not FAILURES else str(len(FAILURES)) + ' OF'} "
      f"{'DEVELOPER LOADING-STATE CHECKS PASSED' if not FAILURES else 'CHECKS FAILED: ' + ', '.join(FAILURES)}")
